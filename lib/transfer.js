...
Transfer.prototype.start = function() {
	...
	pre(this)
	.then(() => {
		if (!this.url) throw new Error('pre function did not set transfer.url');
	})
	.then(() => this.get())
	.catch(err => this.failed(err, true));
};
...
Transfer.prototype.get = function() {
	...
	stream.on('request', req => {
		if (this.log) this.log('Sent HTTP request');
	...
	stream.on('response', res => {
		...
		try {
			...
			if (this.position) {
				...
				if (this.length == null && first) this.length = range[2] * 1;
			} else if (this.length == null && first) {
	...
	const transformStream = new streamModule.Transform({
		transform(chunk, encoding, callback) { // jshint ignore:line
			// If transfer aborted, discard incoming data
	...
	transformStream.on('end', () => {
		...
		if (!this.err && this.position == this.length || (this.length == null && this.res)) { // jshint ignore:line
...
Transfer.prototype.failed = function(err, empty) {
	...
	setTimeout(this.start.bind(this), pause);
};
...
Transfer.prototype.fatal = function() {
	...
	this.stream.emit('error', new errors.TransferError(`Failed - Retries exhausted (${this.attempt}, ${this.attemptTotal} total)`));
