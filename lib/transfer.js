...
Transfer.prototype.get = function() {
	...
	if (this.position > 0) this.gotOptions.headers['accept-encoding'] = 'identity';
	...
	stream.on('response', (_res) => {
		...
		try {
			...
			const {headers} = res;
			if (this.position > 0 || this.length != null) {
				const range = headers['content-range'];
				if (!range) throw new TransferError('No range header');
			...
			} else if (first && headers['content-length'] && !headers['content-encoding']) {
