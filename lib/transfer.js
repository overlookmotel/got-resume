...
function Transfer(options) {
	...
	this.log = options.log || (() => {}); // jshint ignore:line
	...
	stream.on('response', res => {
		...
		try {
			...
			if (this.position) {
				...
				if (range[1] != this.position + '') throw new errors.TransferError(`Server returned wrong range '${range}', expected start at ${this.position}`);
