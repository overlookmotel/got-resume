...
function Transfer(options) {
	...
	this.gotOptions = Object.assign({}, options.got, {encoding: null});
...
Transfer.prototype.start = function() {
	...
	if (this.log) this.log('Calling pre function');
	...
	this.prePromise.then(() => {
		...
		if (this.log) this.log('Completed pre function', {url: this.url});
...
Transfer.prototype.get = function() {
	...
	stream.on('error', (err, body) => {
		if (this.log) this.log('Stream error', {err, body});
	...
	stream.on('response', res => {
		if (this.log) this.log('Received HTTP response', {url: res.url, headers: res.headers});
		...
		try {
			...
			} else if (first && this.length == null && headers['content-length'] && !headers['content-encoding']) {
			...
			if (this.log) this.log('Response error', {err});
			aborted = true;
			this.req.abort(err);
			return;
		}
